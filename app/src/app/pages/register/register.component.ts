  ...
  register() {
    this.authenticationService.registerUser(this.username, this.email, this.password, this.password2).subscribe({
      next: (data) => {
        ...
        this.alertService.addRegistrationAlert(this.username, null, null, "Welcome user !", "Welcome. Your account has been successfully created!", "Your account is by default set as 'Public', with Notifications being 'Enabled'. It is advised to update your profile details and settings", new Date().toISOString(), 'U', 'N', false);
